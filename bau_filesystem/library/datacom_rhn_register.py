...
DOCUMENTATION = '''
---
module: datacom_rhn_register
short_description: Manage Red Hat Network registration using the C(rhnreg_ks) command, patched to support proxy
description:
...
class Rhn(RegistrationBase):
    ...
    def register(self, enable_eus=False, activationkey=None, profilename=None):
        ...
        if self.module.params.get('http_proxy', None):
            register_cmd += " --proxy '%s'" % % self.module.params.get('http_proxy')
...
def main():
    ...
    module = AnsibleModule(
                argument_spec = dict(
                    ...
                    http_proxy = dict(default=rhn.config.get_option('httpProxy'), required=False),
